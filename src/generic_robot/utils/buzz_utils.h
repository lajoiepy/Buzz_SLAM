...
static int buzz_register_hooks() {
   buzz_slam::BuzzSLAMSingleton::GetInstance().GetBuzzSLAM<SLAM>(VM->robot)->RegisterSLAMFunctions(VM);
...
int buzz_script_set(const int& robot_id, 
   ...
   if(buzz_register_hooks<SLAM>() != BUZZVM_STATE_READY) {
      buzzvm_destroy(&VM);
      buzzdebug_destroy(&DBG_INFO);
   ...
   buzz_slam::BuzzSLAMSingleton::GetInstance().GetBuzzSLAM<SLAM>(VM->robot)->Init(VM, gtsam::Point3(), gtsam::Rot3());
   /* All OK */
   return 1;
}
