...
static int BuzzAddSeparator(buzzvm_t vm) {
   ...
   buzzvm_gload(vm);
   /* Call function */
   int is_added = BuzzSLAMSingleton::GetInstance().GetBuzzSLAM<BuzzSLAMRos>(vm->robot)->AddSeparatorMeasurement();
...
static int BuzzLoadDatasetParameters(buzzvm_t vm) {
   ...
   buzzvm_gload(vm);
   /* Call function */
   buzz_slam::BuzzSLAMSingleton::GetInstance().GetBuzzSLAM<buzz_slam::BuzzSLAMRos>(vm->robot)->LoadParameters(dataset_name, sensor_range, outlier_period);
...
static int BuzzReadNextEntry(buzzvm_t vm) {
   buzz_slam::BuzzSLAMSingleton::GetInstance().GetBuzzSLAM<buzz_slam::BuzzSLAMRos>(vm->robot)->AddOdometryMeasurement();
   int number_of_poses = buzz_slam::BuzzSLAMSingleton::GetInstance().GetBuzzSLAM<buzz_slam::BuzzSLAMRos>(vm->robot)->GetNumberOfPoses();
