...
bool BuzzSLAMRos::KeyExists(const gtsam::Key& key) {
...
gtsam::Pose3 BuzzSLAMRos::GetPoseAtKey(const gtsam::Key& key) {
...
void BuzzSLAMRos::LoadParameters(const std::string& dataset_name, const double& sensor_range, const int& outlier_period) {
...
bool BuzzSLAMRos::CompareCentralizedAndDecentralizedError() {
...
void BuzzSLAMRos::ComputeCentralizedEstimate(const std::string& centralized_extension) {
   // Initialize the set of robots on which to evaluate
   std::set<int> robots;
...
void BuzzSLAMRos::ComputeCentralizedEstimateIncremental(std::set<int> robots, const std::string& centralized_extension) {
